...
export default function PolicyWithPaymentsList() {
  ...
  // 🔹 Updated: pass client phone number
  const handlePaymentClick = (payment, clientPhone) => {
    setCurrentPayment({ ...payment, client_phone: clientPhone });
  ...
  try {
    ...
    const minimumPayment = currentPayment.amount_to_be_paid * 0.25;
    if (cleanedInput < minimumPayment) {
      alert(`Payment must be at least 25% of the due amount (₱${minimumPayment.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })})`);
      return;
    }
    ...
    try {
        ...
        "https://ezmvecxqcjnrspmjfgkk.functions.supabase.co/payment-sms",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "Authorization": `Bearer ${"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImV6bXZlY3hxY2pucnNwbWpmZ2trIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTQ1MjUzMzMsImV4cCI6MjA3MDEwMTMzM30.M0ZsDxmJRc7EFe3uzRFmy69TymcsdwMbV54jkay29tI"}` // replace with your anon key
          },
          body: JSON.stringify({
            action: "confirmation",
            payment_id: currentPayment.id,
            paid_amount: cleanedInput
          }),
        }
      );
      ...
      if (!res.ok) {
        ...
        console.error("SMS function returned non-OK response:", res.status, text);
        alert(`SMS failed to send (status ${res.status}). Check console for details.`);
      } else {
        const data = await res.json();
        console.log("SMS Result:", data);
        alert("Payment updated successfully and SMS sent!");
      }
    } catch (smsErr) {
      console.error("Fetch to SMS function failed:", smsErr);
                  ...
                  <table className="payments-table">
                    ...
                    <tbody>
                      {payments.map((p, index) => {
                        ...
                        const isDisabled = status === "fully-paid" || index > firstUnpaidIndex || firstUnpaidIndex === -1;
      ...
      {modalOpen && (
        <div className="payment-modal-backdrop">
          <div className="payment-modal">
            ...
            <p><strong>Amount Due:</strong> {currentPayment?.amount_to_be_paid?.toLocaleString(undefined, { style: "currency", currency: "PHP" })}</p>
            <p><strong>Minimum Payment (25%):</strong> {(currentPayment?.amount_to_be_paid * 0.25)?.toLocaleString(undefined, { style: "currency", currency: "PHP" })}</p>
            <input
              type="text"
              value={paymentInput}
              onChange={(e) => {
